...
def evaluate_NER_model(model, data, test_type): # checked 2024-06-16
    ...
    acc, precision, recall, f1_score, combined_conf_matrix, filtered_acc, filtered_precision, filtered_recall, filtered_f1_score = get_evaluation_result(all_predicted_real_labels, all_detail_labels, all_essays)
    ...
    label_names = list(model.config.label2id.keys()) 
    print("                Predicted")
    print("               ", " ".join([f"{label:^5}" for label in label_names]))

    for i, row in enumerate(combined_conf_matrix):
        print(f"True {label_names[i]:<4} ", " ".join([f"{cell:^5}" for cell in row]))
